...
class AnyShelly:
    def __init__(self, user, config, log: CustomLogger):
    ...
    async def __sync(self):
        while True:
            ...
            if status != self.__last_status:
                ...
                self._log.info('Status=', status)
                run_callbacks(self.__on_data, self.__user, {MEASUREMENT_STATUS: status})
    ...
    def __on_trigger(self, trigger_type):
        try:
            ...
            if trigger_type == TRIGGER_300S:
                ...
                self._log.info('Status=', self.__last_status, ' Power=', self.__power, 'W Energy=', energy, 'Wh')
            if self.__power != self.__last_reported_power:
                self.__last_reported_power = self.__power
                data[MEASUREMENT_POWER] = self.__power
                self._log.info('Power=', self.__power, 'W')
            if data:
                run_callbacks(self.__on_data, self.__user, data)
    ...
    async def __switch(self, on: bool):
        self._log.info('Sending switch request, on=', on)
        if on:
    ...
    async def __get_power(self):
        ...
        if power is None:
            self._log.error(f'No power data available.')
            return 0
        return round(power)
    ...
    async def __get(self, query):
        for i in reversed(range(3)):
            try:
                with ClientSession(self._log, self.__host, self.__port) as session:
                    ...
                    if (status >= 200 and status <= 299):
                        ...
                        self._log.error('Charger query ', query, ' for ', self.__host, ' failed with code ', status, ', ', i, ' retries left.')
            except Exception as e:
                self._log.error('Charger query ', query, ' for ', self.__host, ' failed: ', e, ', ', i, ' retries left.')
                self._log.trace(e)
            await sleep(1)
        return None
