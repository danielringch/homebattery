...
class MicroMqtt():
    ...
    def __on_connect(self):
        ...
        self.__packet_task = create_task(self.__packet_loop())
    ...
    class Frontend:
        ...
        async def __send_subscribe_message(self, packet):
            ...
            pid = pack('!H', packet.pid)
            properties = bytearray(b'\x00')
            topic = self.__string_to_bytes(packet.topic)
            options = pack('!B', 1 << 5 | packet.qos)
            ...
            async with self.__send_lock:
                ...
                packet.mark_sent()
                self.__log.info(f'Outgoing subscription, pid={packet.pid}, qos={packet.qos}: {packet.topic}')
                self.last_tx = time()
        ...
        async def __send_publish_message(self, packet):
            ...
            topic = self.__string_to_bytes(packet.topic)
            pid = pack('!H', packet.pid) if packet.qos > 0 else bytearray()
            properties = bytearray(b'\x00')
            ...
            async with self.__send_lock:
                ...
                packet.mark_sent()
                self.__log.info(f'Outgoing message, pid={packet.pid}, qos={packet.qos}, topic={packet.topic}: {self.bytes_to_hex(data)}')
                self.last_tx = time()
    ...
    class Backend:
        ...
        async def send_string(self, data):
            await self.__socket.send(pack("!H", len(data)))
